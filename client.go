...
func (c *Client) Create(name string) (string, error) {
	...
	body := Jmap{"type": "remote", "url": "https+lxc-images://images.linuxcontainers.org", "name": "lxc-images/ubuntu/trusty/amd64"}
	...
	if resp.Type != Async {
		return "", fmt.Errorf("Got a non-async response from create")
	}
